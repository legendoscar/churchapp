<?php session_start();
    if($_SERVER['REQUEST_METHOD'] === "POST") {
        ...
        if(is_authorized($account_type, "delete-payable-item", "", "") === "allowed"){ // ENSURE USER HAS ADMIN PREVILLEGE 
            if( isset($_POST['item_id']) && !empty($_POST['item_id']) ){
                ...
                if(get_payable_items_data_by_id($item_id, "name") != "not_found"){//Is Existing
                    ...
                    //Save New Product
                    $status = "deleted";
                    $update = @mysqli_prepare($conn, "UPDATE payable_items SET `status`=?  where id=?");
