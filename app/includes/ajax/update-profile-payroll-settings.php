<?php session_start();
    if($_SERVER['REQUEST_METHOD'] === "POST") {
        ...
        if(is_authorized($account_type, "edit-own-basic-salary", "", "") === "allowed"){

            if( isset($_POST['user_has_payroll']) && !empty($_POST['user_has_payroll']) ){
                ...
                if(get_user_data($account_id, "acc_id") != $account_id) {// IS EMPLOYEE ID VALID
                ...
                $generate = mysqli_prepare($conn, "UPDATE user_accounts SET basic_salary=?, bank_id=?, bank_name=?, account_number=?, account_name=? where acc_id=?");
                mysqli_stmt_bind_param($generate, "sisssi", $basic_salary, $bank_id, $bank_name, $account_number, $account_name, $account_id);
