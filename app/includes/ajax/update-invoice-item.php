        ...
        try {
            if(is_authorized($account_type, "edit-receipt", "", "") === "allowed"){
                if(
                    ...
                    $update = @mysqli_prepare($conn, "UPDATE invoice_trn_details SET `product_id`=?, rate=?, amount=? where id=? and invoice_id=? and invoice_number=?");
                    @mysqli_stmt_bind_param($update, "issiis", $invoice_item, $invoice_item_amount, $invoice_item_amount, $invoice_row_id, $invoice_id, $invoice_number);
                    if(mysqli_stmt_execute($update)){
                        ...
                        if(mysqli_stmt_execute($update)){
                            ...
                            <script>
                                $("#row_id_<?php echo $invoice_row_id;?> select").html("<option value='' label='Choose Product'><?php echo get_payable_items_data_by_id($invoice_item, "name");?></option>");
                                $("#row_id_<?php echo $invoice_row_id;?> div.input-group input").val("<?php echo $invoice_item_amount;?>");
                                autoCals();
