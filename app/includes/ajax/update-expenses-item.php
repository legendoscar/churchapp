<?php session_start();
    if($_SERVER['REQUEST_METHOD'] === "POST") {
        ...
        if(is_authorized($account_type, "edit-expenses-item", "", "") === "allowed" ){ // ENSURE USER HAS ADMIN PREVILLEGE
            if(
                ...
                if(get_expenses_item_data_by_id($item_id, "name") != "not_found"){//Is Existing

                    $update = @mysqli_prepare($conn, "UPDATE expenses_item SET `name`=?  where id=?");
                    @mysqli_stmt_bind_param($update, "si", $item_name, $item_id);
                    if(mysqli_stmt_execute($update)){

                        echo server_response("success", "Item has been updatedss!", "100");?>
            ...
            echo server_response("error", "<b>Access Denied!</b> You're not allowed to update this brand. Please if you think this was a mistake, contact your administrator.", "100");
        }
