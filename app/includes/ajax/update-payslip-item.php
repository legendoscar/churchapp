<?php session_start();
    if($_SERVER['REQUEST_METHOD'] === "POST") {
        ...
        if(is_authorized($account_type, "edit-payslip-item", "", "") === "allowed"){
            if(
                ...
                if(get_payslip_items_data_by_id($item_id, "name") != "not_found"){//Is Existing
                    ...
                    if(get_payslip_items_data_by_id($item_id, "is_default") == "default") {
                        exit(server_response("error", "Default items can not be modified!", "100"));
                    }

                    $slug_name = seo_friendly_url($item_name);

                    $update = @mysqli_prepare($conn, "UPDATE payslip_items SET `name`=?, slug_name=?  where id=?");
                    @mysqli_stmt_bind_param($update, "ssi", $item_name, $slug_name, $item_id);
                    if(mysqli_stmt_execute($update)){

                        echo server_response("success", "Item has been updated!", "100");?>
