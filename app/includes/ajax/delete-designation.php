<?php session_start();
    if($_SERVER['REQUEST_METHOD'] === "POST") {
        ...
        if(is_authorized($account_type, "delete-designation", "", "") === "allowed"){ // ENSURE USER HAS ADMIN PREVILLEGE 
            if( isset($_POST['item_id']) && !empty($_POST['item_id']) ){
                ...
                if(get_designation_data_by_id($item_id, "name") != "not_found"){//Is Existing

                    //Save New Product
                    $status = "deleted";
                    $update = @mysqli_prepare($conn, "UPDATE designations SET `status`=?  where id=?");
                    ...
                    if(mysqli_stmt_execute($update)){
                        ...
                        <script>
                            ...
                            $("#total-items").html("<?php echo number_format(get_designation_list('count'));?>");
                        </script>
