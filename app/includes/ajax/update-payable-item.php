<?php session_start();
    if($_SERVER['REQUEST_METHOD'] === "POST") {
        ...
        if(is_authorized($account_type, "edit-payable-item", "", "") === "allowed"){ // ENSURE USER HAS ADMIN PREVILLEGE
            if(
                ...
                if(get_payable_items_data_by_id($item_id, "name") != "not_found"){//Is Existing

                    $update = @mysqli_prepare($conn, "UPDATE payable_items SET `name`=?  where id=?");
