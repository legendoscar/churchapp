<?php session_start();
    if($_SERVER['REQUEST_METHOD'] === "POST") {
        ...
        if(is_authorized($account_type, "create-payslip-item", "", "") === "allowed"){ 
            if(
                ...
                $item_type = SanitizeTEXT(mysqli_real_escape_string($conn, $_POST['item_type']));

                if(!in_array($item_type, array("earnings", "deduction"))) { exit(server_response("success", "Item successfully created!", "100")); }

                $group = $item_type;

                $created_by = $account_id;
                $url_id = md5(password_hash(rand(200, 1000), PASSWORD_DEFAULT));
                $slug_name = seo_friendly_url($item_name);

                $create = @mysqli_prepare($conn, "INSERT INTO payslip_items(`name`, group_name, slug_name, created_by, url_id) value(?,?,?,?,?)");
                @mysqli_stmt_bind_param($create, "sssss", $item_name, $group, $slug_name, $created_by, $url_id);
                if(mysqli_stmt_execute($create)){
                    ...
                    <script>
                        ...
                        $("#total-items").html("<?php echo number_format(get_payslip_items_list("count", "$item_type"));?>");
