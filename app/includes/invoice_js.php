...
<script>
    ...
    function NewTableRow(){
        var dataString = "rows_number=" + $("#rows_number").val()+"&discount_pattern=" + $("#pattern").val();
        $.ajax({
            type: "POST",
            url: "./includes/ajax/add-row",
            data: dataString,
            cache: false,
            beforeSend: function() {
                $("button#add_more_btn").html("<span class='fa fa-spin fa-spinner'></span> Adding... Please wait.");
                $("button#add_more_btn").attr("disabled", "disabled");
            },
            success: function(d) {
                $('div#return_server_msg').fadeIn('slow').html(d);
                $("button#add_more_btn").html("<span class='fa fa-plus-circle'></span> Add More Item(s)");
                $("button#add_more_btn").removeAttr("disabled", "disabled");
            },
            error: function(d) {
    ...
    function autoCals() {
        ...
        // TOTAL AMOUNT
        var elems_amount_total = document.getElementsByClassName('amount');
        ...
        // document.getElementById('total_paid').innerHTML = new_val.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",").valueOf();
        // document.getElementById('total_paid').innerHTML = total_amount_each.toFixed(3).toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",").valueOf();
        // document.getElementById('product_total').innerHTML = total_amount_each.toFixed(3).toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",").valueOf();
        // document.getElementById('total_invoice_value').innerHTML = new_val.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",").valueOf();
        // document.getElementById('total_invoice_value').innerHTML = total_amount_each.valueOf().toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
        // document.getElementById('h_qty').value = total_qty.toString();
